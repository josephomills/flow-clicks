<div>
    ...
    <form wire:submit.prevent="save" class="flex space-x-6">
        <div class="grid gap-6 mb-6 bg-background rounded-md border p-6">
            {{-- First Name field --}}
            <div>
                ...
                <select id="denomination" wire:model.defer="denomination"
                    ...
                    @foreach ($denominations as $denomination)
                        <option value="{{ $denomination->id }}"
                            {{ $user->defaultDenomination->id == $denomination->id ? 'selected' : '' }}>
                            {{ $denomination->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" id="role" wire:model.defer="role" class="sr-only peer"
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6 w-full max-w-md">
            ...
            @if ($show_dropdown)
                <div wire:click="hideDropdown" class="fixed inset-0 z-40"></div>
            @endif
        </div>
