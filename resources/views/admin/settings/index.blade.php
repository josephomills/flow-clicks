...
@section('content')
  <div>
    ...
    <form method="post" action="{{ route('admin.settings.store') }}" class="mt-6 space-y-6">
      ...
      <input type="text" id="userID" name="userID" value="{{ $current_user->id }}"
      ...
      <div>
        ...
        <input type="text" id="country" name="name" value="{{ $current_user->name }}"
        class="w-full h-10 rounded-md border border-input bg-background px-3 py-2 font-mono text-sm" required
      ...
      <option value="{{ $denomination->id }}" {{ $current_user->denomination_id == $denomination->id ? 'selected' : '' }}>
        {{ $denomination->name }}
      </option>
      @endforeach
